...
def create_tmp_dimensional_tables_shares(dl_bucket_input_prefix, spark):
    """
    This function creates 2 dataframes (tmp_financial_report_dim,tmp_tickers_shares_fact) from tickers
    shares related data fetched from input folder.
    :param dl_bucket_input_prefix: prefix of the input folder
    :param spark: spark object
    :return: tmp_financial_report_dim,tmp_tickers_shares_fact
    """
    #input path
    ...
    tickers_shares_parquet_path = os.path.join(tickers_shares_path,'tickers_shares.parquet')
    
    #input df
    tickers_shares_sp_df = spark.read.parquet(tickers_shares_parquet_path) 
    tickers_shares_sp_df = tickers_shares_sp_df.withColumn('period', lit('year'))
        
    #tmp_financial_report_dim
    tmp_financial_report_dim = tickers_shares_sp_df.select('year',col('symbol').alias('ticker'),'period').\
        dropDuplicates().\
        withColumn('reporting_id',concat_ws('_','ticker','year','period'))
    
    #tmp_tickers_shares_fact
    tmp_tickers_shares_fact = tickers_shares_sp_df.\
        withColumn('reporting_id',concat_ws('_',col('symbol').alias('ticker'),'year','period'))
    tmp_tickers_shares_fact = tmp_tickers_shares_fact.select('reporting_id','basic_shares')
...
def main():
    ...
    tmp_financial_report_dim,tmp_tickers_shares_fact =\
        create_tmp_dimensional_tables_shares(dl_bucket_input_prefix = DL_BUCKET_INPUT_PREFIX,\
                                          spark = spark)
    ...
    print('tmp_tickers_shares_fact')
    print(tmp_tickers_shares_fact.show(5))
    
    #output path
    tickers_shares_fact_path = os.path.join(DL_BUCKET_OUTPUT_PREFIX,'tickers_shares_fact/')
    write_files = os.path.join(tickers_shares_fact_path,'tickers_shares_fact.parquet')
    read_files = os.path.join(tickers_shares_fact_path,'tickers_shares_fact_tmp.parquet')
    
    print('update tickers_shares_fact in datalake')
    refresh_parquet_files (write_files = write_files,\
                           read_files = read_files,\
                           tmp_df = tmp_tickers_shares_fact,\
                           spark = spark,\
                           s3_client = s3_client,\
                           dl_bucket_name = DL_BUCKET_NAME,
                           dl_bucket_output_prefix = 'datalake/tickers_shares_fact/tickers_shares_fact.parquet/',
                           # dl_bucket_output_prefix_0=DL_BUCKET_OUTPUT_PREFIX,
                           # dim_table_name='tickers_shares_fact',
                           columns_list=['reporting_id']
                          )
